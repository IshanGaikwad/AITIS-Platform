...
import type { JiraIssue, SavedStory } from "@/lib/types";
import type {
  AutomationArtifact,
  CoverageSummary,
  Framework,
...
export default function TestGeneratorPage() {
  ...
  function handleApplyAcMapping() {
    ...
    setStory((current) => ({ ...current, acceptanceCriteria: cleaned }));
  }
  ...
  async function handleSaveStory() {
    ...
    setError(null);
    try {
      const payload: Story = { ...story, acceptanceCriteria: textToCriteria(acceptanceCriteriaText) };
      if (selectedStoryId) {
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(event) =>
                    updateStoryField("framework", event.target.value as Framework)
                  }
                  className="w-full rounded-2xl border border-slate-200 px-4 py-2.5 outline-none transition focus:border-slate-400"
                >
          ...
          >
            <div className="space-y-3 text-sm text-slate-700">
              <div className="rounded-2xl bg-slate-50 p-4">
                ...
                {coverage?.totalAcceptanceCriteria ?? stats.acceptanceCriteriaCount}
              </div>
              <div className="rounded-2xl bg-slate-50 p-4">
            ...
            >
              <div className="rounded-3xl bg-slate-950 p-5 text-sm text-slate-100">
                <pre className="overflow-x-auto whitespace-pre-wrap">
                  {JSON.stringify(
                    { ...story, acceptanceCriteria: textToCriteria(acceptanceCriteriaText) },
                    null,
                    2,
                  )}
                </pre>
